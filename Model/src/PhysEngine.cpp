        ...
        for (int j = i + 1; j < particles->particle_list.size(); j++) {
            if (check_collission(particles->particle_list[i], particles->particle_list[j])) {
                ...
                if (TE_post_ij != TE_pre_ij) {
                    ...
                    // cout << "overlap introduced energy error before scaling: " << (TE_post_ij - TE_pre_ij) / TE_pre_ij << endl;
                    
                    // high_prec energy_correction_factor = sqrt(TE_pre_ij / TE_post_ij);

                    // particles->particle_list[i]->vx = (vx1_new * energy_correction_factor).convert_to<double>();
                    // particles->particle_list[i]->vy = (vy1_new * energy_correction_factor).convert_to<double>();
                    // particles->particle_list[j]->vx = (vx2_new * energy_correction_factor).convert_to<double>();
                    // particles->particle_list[j]->vy = (vy2_new * energy_correction_factor).convert_to<double>();
                    ...
                    // //if (TE_error_ij_corrected > error_threshold) {
                    // cout << "TE error after scaling remains: " << TE_error_ij_corrected << endl;
                    // //}
                    ...
                    // Define delta_v1n as the adjustment to v1n
